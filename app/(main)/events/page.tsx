  ...
  return (
    <motion.button layout initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96 }} whileHover={{ y: -4, boxShadow: "0 16px 40px rgba(0,0,0,0.13)" }} transition={{ duration: 0.22 }} onClick={() => onSelect(event)}
      ...
      <div style={{ marginTop: "auto", padding: "10px 18px", borderTop: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: event.accent_color }}>Details →</span>
      </div>
    </motion.button>
  );
}
...
function DetailModal({ event, onClose }: { event: DBEvent; onClose: () => void }) {
  ...
  const lineup = event.lineup || [];
  const lLabel = lineupLabel(lineup);
  const tabs = [{ id: "overview" as TabId, label: "Overview" }, ...(lineup.length > 0 ? [{ id: "lineup" as TabId, label: lLabel }] : []), ...((event.schedule?.length || 0) > 0 ? [{ id: "schedule" as TabId, label: "Schedule" }] : []), ...((event.prizes?.length || 0) > 0 ? [{ id: "prizes" as TabId, label: "Prizes" }] : [])];
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={e => e.target === e.currentTarget && onClose()}
      ...
      <motion.div initial={{ opacity: 0, y: 28, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: 0.22 }}
        style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 620, maxHeight: "calc(100vh - 100px)", overflowY: "auto", boxShadow: "0 32px 80px rgba(0,0,0,0.25)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div style={{ flex: 1, marginRight: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <span style={{ padding: "2px 9px", borderRadius: 999, fontSize: 10, fontWeight: 800, background: event.accent_color + "18", color: event.accent_color, border: `1px solid ${event.accent_color}28`, letterSpacing: "0.06em" }}>{CATEGORIES.find(c => c.id === event.category)?.label.toUpperCase()}</span>
                <StatusPill status={event.status} />
              </div>
        ...
        <div style={{ padding: "20px 22px" }}>
          {tab === "overview" && <>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: (event.sponsors?.length || 0) > 0 ? 16 : 0 }}>{(event.tags || []).map(t => <span key={t} style={{ padding: "4px 12px", borderRadius: 999, fontSize: 12, fontWeight: 600, background: event.accent_color + "14", color: event.accent_color }}>{t}</span>)}</div>
            {(event.sponsors?.length || 0) > 0 && <div style={{ marginTop: 12 }}><p style={{ margin: "0 0 6px", fontSize: 10, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.08em" }}>Sponsors & Partners</p><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{event.sponsors.map(s => <span key={s} style={{ padding: "4px 12px", background: "#f3f4f6", borderRadius: 6, fontSize: 12, color: "#374151", fontWeight: 500 }}>{s}</span>)}</div></div>}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {(event.social_instagram || event.social_facebook) && (
        <div style={{ display: "flex", gap: 8, marginTop: 2 }}>
          ...
          {tab === "lineup" && <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{lineup.map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, background: "#f9fafb", borderRadius: 12, padding: "12px 16px" }}>
              {item.photo_url ? <img src={item.photo_url} alt={item.name} style={{ width: 46, height: 46, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} /> : <AvatarCircle initials={item.avatar_initials || item.name.slice(0, 2).toUpperCase()} color={item.avatar_color || "#374151"} size={46} />}
              <div style={{ flex: 1 }}><p style={{ margin: 0, fontWeight: 700, fontSize: 15, color: "#111827" }}>{item.name}</p><p style={{ margin: "2px 0 0", fontSize: 12, color: "#6b7280" }}>{[item.sub_role, item.genre, item.company, item.team, item.origin ? `From ${item.origin}` : null].filter(Boolean).join(" · ")}</p>{item.topic && <span style={{ display: "inline-block", marginTop: 4, padding: "2px 8px", background: event.accent_color + "14", color: event.accent_color, borderRadius: 999, fontSize: 11, fontWeight: 600 }}>"{item.topic}"</span>}</div>
              {item.upcoming_shows !== undefined && <div style={{ textAlign: "right" }}><p style={{ margin: 0, fontSize: 20, fontWeight: 900, color: item.avatar_color || event.accent_color }}>{item.upcoming_shows}</p><p style={{ margin: 0, fontSize: 10, color: "#9ca3af" }}>upcoming</p></div>}
            </div>
          ))}</div>}
          {tab === "schedule" && <div style={{ display: "flex", flexDirection: "column" }}>{(event.schedule || []).map((item, i) => (
            <div key={i} style={{ display: "flex", gap: 14, paddingBottom: 16 }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}><div style={{ width: 10, height: 10, borderRadius: "50%", background: event.accent_color, marginTop: 4, flexShrink: 0 }} />{i < (event.schedule?.length ?? 0) - 1 && <div style={{ width: 1, flex: 1, background: "#e5e7eb", marginTop: 4 }} />}</div>
              <div style={{ flex: 1 }}><p style={{ margin: "0 0 2px", fontSize: 10, fontWeight: 700, color: event.accent_color, letterSpacing: "0.05em" }}>{item.slot_label}</p><p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: "#111827" }}>{item.title}</p>{item.speaker_name && <p style={{ margin: "2px 0 0", fontSize: 12, color: "#6b7280" }}>by {item.speaker_name}</p>}</div>
            </div>
          ))}</div>}
          {tab === "prizes" && <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{(event.prizes || []).map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, background: i === 0 ? "#fffbeb" : "#f9fafb", borderRadius: 10, padding: "12px 16px", border: `1px solid ${i === 0 ? "#fde68a" : "#f3f4f6"}` }}>
              ...
              <div><p style={{ margin: 0, fontWeight: 700, fontSize: 14, color: "#111827" }}>{item.rank_label}</p><p style={{ margin: "2px 0 0", fontSize: 12, color: "#6b7280" }}>{item.reward}</p></div>
            </div>
          ))}</div>}
        </div>
        <div style={{ padding: "14px 22px", borderTop: "1px solid #f3f4f6", display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", position: "sticky", bottom: 0, background: "#fff" }}>
          {event.status !== "past" && event.status !== "cancelled" && <button onClick={() => { onClose(); if (event.register_href) router.push(event.register_href); }} style={{ padding: "11px 22px", background: event.accent_color, color: "#fff", border: "none", borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: "pointer", flex: 1 }}>{event.status === "postponed" ? "Get Notified" : "Register Now →"}</button>}
          ...
          {/* <span style={{ fontSize: 12, color: "#9ca3af", marginLeft: "auto" }}>Contact: +91 70855 71865</span> */}
    ...
    >
      ...
      {[
        ...
        <motion.div
          ...
          transition={{ delay: 0.3 + item.delay, duration: 0.4, type: "spring" }}
          style={{
            position: "absolute",
            top: item.top,
            left: item.left,
      ...
      <div style={{ minHeight: "100vh", background: "#f0f2f5", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
              ...
              <div style={{ flex: "1 1 380px" }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    ...
                    <a href="/delivery" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 18px", background: "#6ee7b7", border: "1px solid transparent", borderRadius: 10, color: "#064e3b", fontSize: 13, fontWeight: 700, textDecoration: "none" }}>
                      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="hero-right-panel" style={{ flex: "0 0 340px", maxWidth: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
                {featured.length > 0 ? (
                  <>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {featured.map(ev => {
                        const cat = CATEGORIES.find(c => c.id === ev.category); return (
                          <motion.button key={ev.id} whileHover={{ x: 3 }} onClick={() => setSelectedEvent(ev)} style={{ all: "unset", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,0.07)", borderLeft: `3px solid ${ev.accent_color}`, borderRadius: "0 10px 10px 0", padding: "10px 14px", textAlign: "left" }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}><p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#fff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{ev.title}</p><p style={{ margin: "1px 0 0", fontSize: 11, color: "#9ca3af" }}>{ev.date_start} · {ev.city}</p></div>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", position: "sticky", top: 0, zIndex: 100, marginTop: banners.length > 0 ? 24 : 0, overflowX: "auto" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", padding: "0 16px" }}>
            {CATEGORIES.map(cat => {
              const active = activeCategory === cat.id; return (
                <button key={cat.id} onClick={() => setActiveCategory(cat.id)} style={{ padding: "13px 12px", background: "none", border: "none", borderBottom: `2px solid ${active ? cat.accent : "transparent"}`, color: active ? cat.accent : "#6b7280", fontWeight: active ? 700 : 500, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 5, transition: "all 0.15s" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 20px 64px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22, gap: 10, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
              {(["all", "upcoming", "open", "past", "postponed"] as const).map(s => <button key={s} onClick={() => setStatusFilter(s)} style={{ padding: "5px 11px", borderRadius: 999, border: `1px solid ${statusFilter === s ? "#111827" : "#e5e7eb"}`, background: statusFilter === s ? "#111827" : "#fff", color: statusFilter === s ? "#fff" : "#6b7280", fontSize: 11, fontWeight: 600, cursor: "pointer", textTransform: "capitalize" }}>{s === "all" ? "All Status" : s}</button>)}
              <select value={sortBy} onChange={e => setSortBy(e.target.value as "date" | "name")} style={{ padding: "5px 10px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff", fontSize: 12, color: "#374151", fontWeight: 600, cursor: "pointer" }}><option value="date">Date</option><option value="name">Name</option></select>
              {(activeCategory !== "all" || statusFilter !== "all" || search) && <button onClick={() => { setActiveCategory("all"); setStatusFilter("all"); setSearch(""); }} style={{ padding: "5px 12px", borderRadius: 8, border: "none", background: "#fee2e2", color: "#991b1b", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Clear</button>}
            </div>
          </div>
          ...
          {/* <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ marginTop: 48, background: "#0f172a", borderRadius: 20, padding: "36px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 24, flexWrap: "wrap" }}>
            <div><h2 style={{ margin: "0 0 6px", fontSize: 22, fontWeight: 800, color: "#fff" }}>Organising an event in Manipur?</h2><p style={{ margin: 0, fontSize: 14, color: "#94a3b8" }}>List your event and reach thousands across Northeast India.</p></div>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <a href="/admin/events/new" style={{ display: "inline-block", padding: "12px 22px", background: "#6ee7b7", color: "#064e3b", borderRadius: 10, fontWeight: 700, fontSize: 14, textDecoration: "none" }}>Submit Your Event →</a>
              <a href="/edufest" style={{ display: "inline-block", padding: "12px 22px", background: "rgba(255,255,255,0.07)", color: "#fff", borderRadius: 10, fontWeight: 700, fontSize: 14, textDecoration: "none", border: "1px solid rgba(255,255,255,0.18)" }}>EduFest Registration</a>
            </div>
          </motion.div> */}
        </div>
