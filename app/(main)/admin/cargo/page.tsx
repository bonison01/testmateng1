...
import { Loader2, Search, X, ChevronDown, ChevronUp, FileText, Plus } from "lucide-react";
...
export default function AdminBookingsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (deliveryMode !== "all") {
      rows = rows.filter((b) => b.delivery_mode === deliveryMode);
    }
    if (startDate) {
      rows = rows.filter((b) => new Date(b.created_at) >= new Date(startDate));
    }
    if (endDate) {
    ...
    return rows;
  }, [bookings, search, status, deliveryMode, startDate, endDate, sortKey, sortDir]);
  ...
  const hasActiveFilters =
    search || status !== "all" || deliveryMode !== "all" || startDate || endDate;
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3 rounded-lg border border-neutral-200 p-3">
        ...
        <Input
          ...
          className="w-[150px] bg-white text-neutral-900 placeholder:text-neutral-400 placeholder:opacity-60 border-neutral-300"
        />
        <Input
          type="date"
          value={endDate}
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              filtered.map((b) => (
                <tr key={b.id} className="border-b border-neutral-100 last:border-0 hover:bg-emerald-50/40">
                  <td className="px-4 py-2.5 font-mono text-xs text-neutral-700">{b.tracking_id}</td>
