...
export default function CargoBookingPage() {
  ...
  // Autofill sender from user metadata
  useEffect(() => {
    if (user) {
  ...
  // Auto fetch city/state for sender PIN
  useEffect(() => {
  ...
  // Auto fetch city/state for receiver PIN
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // const rate: Rate | undefined = rateMap[receiverPincode];
    const routeKey = `${formData.senderPincode}-${formData.receiverPincode}`;
const rate: Rate | undefined = rateMap[routeKey];
    ...
    const pickupCharge = formData.pickupRequired ? 30 : 0;
    const deliveryCharge = formData.deliveryRequired ? 40 : 0;

    const total = base + pickupCharge + deliveryCharge + (handlingCharge || 0) + (docketCharge || 0);
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // TODO: Upload to Supabase Storage and replace URL accordingly
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!session?.user) {
        toast.error("You must be logged in to book cargo.");
